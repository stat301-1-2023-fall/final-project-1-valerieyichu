...
::: {.callout-tip icon="false"}
This dataset contains six million ratings for ten thousand most popular (with most ratings) books.
:::
...
And looking at the correlation confirms this: The correlation between `rank_of_number_ratings` and`average_rating` is almost 0.
...
-   
    ...
    3.  The number of people who gave books a decently high rating tend to pull up average_rating from the people who rated that book lowly.
...
- Each user (user_id) can give one rating to one book. 
...
These are the top 10 users (of 53,424 total users who rated the top 10,000 rated books on Goodreads) who rated the most books in this Goodreads dataset.
...
```{r}
# We just added a new variable, `average_rating` to the "books_and_ratings" dataset. `average_rating` shows the average rating of each user.
...
# As the table above shows, the users who left the most ratings left 200 ratings. So let's use filter to keep only the users who rated more than 195 books. 
...
```{r}
books_and_ratings_top_raters |> 
  ggplot(aes(x = user_id, y = average_rating, color = average_rating)) +
  geom_point() +
  ylim(c(2, 5)) +
  scale_y_continuous(breaks = seq(2, 5, by = 0.125))
