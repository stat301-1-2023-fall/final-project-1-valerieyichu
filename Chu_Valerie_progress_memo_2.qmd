...
```{r}
anti_join(books, ratings, by = c("work_id" = "book_id")) |> distinct(work_id)
```

The code above returns all rows for `work_id` within books that don't have a matching `book_id` within rating.

```{r}
anti_join(books, ratings, by = c("work_id" = "book_id")) |> distinct(work_id)
```

The code above returns all rows for `book_id` within rating that don't have a matching `work_id` within books. 

Interestingly, both antijoins reveal that there are 9,824 rows missing. So that means that `book_id` from rating and `work_id` from books have the same number of implicitly missing values. I wonder why that is. 
